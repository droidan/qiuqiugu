...
public class JCVideoPlayerStandard extends JCVideoPlayer {
    ...
    public boolean setVideoInfo(String url, int screen, Object... objects) {
        ...
        if (super.setVideoInfo(url, screen, objects)) {
            titleTextView.setText(objects[0].toString());
            if (currentScreen == SCREEN_WINDOW_FULLSCREEN) {
                fullscreenButton.setImageResource(R.mipmap.jc_shrink);
                backButton.setVisibility(View.VISIBLE);
                tinyBackImageView.setVisibility(View.INVISIBLE);
            } else if (currentScreen == SCREEN_LAYOUT_NORMAL
                    || currentScreen == SCREEN_LAYOUT_LIST) {
                fullscreenButton.setImageResource(R.mipmap.jc_enlarge);
...
//                    }
    ...
    public void showProgressDialog(float deltaX, String seekTime, int seekTimePosition, String totalTime, int totalTimeDuration) {
        ...
        if (mProgressDialog == null) {
            ...
            localLayoutParams.y = getResources().getDimensionPixelOffset(com.medialibrary.R.dimen.jc_progress_dialog_margin_top);
