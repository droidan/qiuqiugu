package com.medialibrary.video.utils;
 ...
 * 防止Handler内存泄漏
 * handler内存泄漏原理：
 * handler postDelayed 时会保持对当前acitivity的引用，一直到消息处理完成，这样就会导致当前Activity无法销毁
 */
public class HandlerWeak {
     ...
     */
    public HandlerWeak(Handler.Callback callback) {
     ...
     */
    public HandlerWeak(Looper looper) {
        mCallback = null;
     ...
     * @param callback The callback interface in which to handle messages, or null.
     */
    public HandlerWeak(Looper looper, Handler.Callback callback) {
        mCallback = callback;
