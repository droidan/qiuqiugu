...
/**
 * Created by jian_zhou on 2016/7/19.
 */
public class RxPermissions {
            ...
            for (String permission : permissions) {
                if (!rxPermissions.isGranted(permission)) {
                    if (notGrantedPermissions == null) {
                        notGrantedPermissions = new ArrayList<String>(permissions.length);
                    }
    ...
    protected RxPermissions(Context context) {
        ...
        subjects = new HashMap<String, PublishSubject<Boolean>>(6);
    }
    ...
    @MainThread
    @NonNull
    public Observable<Boolean> observe(String... permissions) {
        List<Observable<Boolean>> observables = new ArrayList<Observable<Boolean>>(permissions.length);
        for (String permission : permissions) {
    ...
    protected boolean isGranted(String permission) {
     ...
     private boolean isGranted60(String permission) {
