...
import com.tiger.socol.gu.activity.community.ColorFilterImageView;
...
public class MultiImageView extends LinearLayout {
    ...
    private ImageView createImageView(int position, final boolean isMultiImage) {
        ...
        if (photoInfo.resid == 0) {
            imageView.setBackgroundColor(getResources().getColor(R.color.im_font_color_text_hint));
            Picasso.with(getContext()).load(photoInfo.url).into(imageView);
        } else {
            Picasso.with(getContext()).load(photoInfo.resid).into(imageView);
        }
        return imageView;
    }
